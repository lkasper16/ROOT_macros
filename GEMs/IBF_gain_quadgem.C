...
//Channel 7 and 8 for new set of gems, channel 8 & 9 for old set of gems
void IBF_gain_quadgem(){
  ...
  filename[1] << "XRay_inten_vary_qgemwscreen/PA_14-26_dvgem345V_arco2_7030_Xray_20kV_5uA.txt";
  ifs[1].open(filename[1].str());
  
  filename[2] << "XRay_inten_vary_qgemwscreen/PA_14-47_dvgem345V_arco2_7030_Xray_20kV_7uA.txt";
  ifs[2].open(filename[2].str());
  
  filename[3] << "XRay_inten_vary_qgemwscreen/PA_15-01_dvgem345V_arco2_7030_Xray_20kV_9uA.txt";
  ifs[3].open(filename[3].str());

  filename[4] << "XRay_inten_vary_qgemwscreen/PA_15-54_dvgem345V_arco2_7030_Xray_20kV_11uA.txt";
  ifs[4].open(filename[4].str());
  
  filename[5] << "XRay_inten_vary_qgemwscreen/PA_16-39_dvgem345V_arco2_7030_Xray_20kV_13uA.txt";
  ...
  for (int i = 1 ; i < N; i++){
    ...
    gtemp_sc[i] = new TGraph();
    temp_sc= Form("gtemp_sc%d", i);
    gtemp_sc[i]->SetName(temp_sc);
	...
	//channel 1,2,3 used for screen, drfit cathode and anode respectively
	gtemp_sc[i]->SetPoint(gtemp_sc[i]->GetN(), t[i] , g1i[i]);
	gtemp_cat[i]->SetPoint(gtemp_cat[i]->GetN(), t[i] , g2i[i]);
	gtemp_an[i]->SetPoint(gtemp_an[i]->GetN(), t[i] , g3i[i]);
    }
    ...
    xray_sc = Form("fxray_sc%d", i);
    fxray_sc[i] = new TF1(xray_sc, "pol0", 200, 500);
    fxray_sc[i]->SetName(xray_sc);
    fxray_sc[i]->SetLineColor(6);
    ...
    fe55_sc = Form("ffe55_sc%d", i);
    ffe55_sc[i] = new TF1(fe55_sc, "pol0", 0, 50);
    ffe55_sc[i]->SetName(fe55_sc);
    ffe55_sc[i]->SetLineColor(6);
  }
    ...
    g_sc[1]->Fit("fxray_sc1","R");

    g_an[1]->Fit("ffe55_an1","R+");
    g_cat[1]->Fit("ffe55_cat1", "R+");
    g_sc[1]->Fit("ffe55_sc1", "R+");
    ...
    g_sc[2]->Fit("fxray_sc2", "R");
    g_an[2]->Fit("ffe55_an2","R+");
    g_cat[2]->Fit("ffe55_cat2", "R+");
    ...
    legend1_1->AddEntry(g_an[1],"X ray I = 7 uA, V = 20 kV", "");
    legend1_1->Draw();

    c0->cd(3);
    TH1D *h3 = new TH1D("h3","h3", 1000, -0.1, 999.9);
    h3->SetTitle("Vin =  3600 V");
    ...
    g_sc[3]->Fit("fxray_sc3","R");
    g_an[3]->Fit("ffe55_an3","R+");
    g_cat[3]->Fit("ffe55_cat3", "R+");
    g_sc[3]->Fit("ffe55_sc3", "R+");
    ...
    legend1_2->AddEntry(g_an[1],"X ray I = 9 uA, V = 20 kV", "");
    legend1_2->Draw();
    
    c0->cd(4);
    TH1D *h4 = new TH1D("h4","h4", 1000, -0.1, 999.9);
    h4->SetTitle("Vin =  3650 V");
    ...
    g_sc[4]->Fit("fxray_sc4", "R");
    g_an[4]->Fit("ffe55_an4","R+");
    g_cat[4]->Fit("ffe55_cat4", "R+");
    g_sc[4]->Fit("ffe55_sc4", "R+");
    g_an[4]->Draw("sameL");
    g_cat[4]->Draw("sameL");
    g_sc[4]->Draw("sameL");
    ...
    legend1_3->AddEntry(g_an[1],"X-Ray I = 11 uA, V = 20 kV","");
    legend1_3->Draw();
    
    c0->cd(5);
    TH1D *h5 = new TH1D("h5","h5", 1000, -0.1, 999.9);
    h5->SetTitle("Vin =  3700 V");
    h5->GetYaxis()->SetRangeUser(-0.5, 3.5);
    h5->GetXaxis()->SetTitle("Time [seconds]");
    h5->GetYaxis()->SetTitle("current [nA]");
    h5->Draw();

    g_an[5]->Fit("fxray_an5","R");
    g_cat[5]->Fit("fxray_cat5", "R");
    g_sc[5]->Fit("fxray_sc5", "R");
    g_an[5]->Fit("ffe55_an5","R+");
    g_cat[5]->Fit("ffe55_cat5", "R+");
    g_sc[5]->Fit("ffe55_sc5", "R+");
    g_an[5]->Draw("sameL");
    g_cat[5]->Draw("sameL");
    g_sc[5]->Draw("sameL");

    TLegend *legend1_4 = new TLegend(0.55,.30,.75,0.5, NULL, "brNDC");
    legend1_4->SetFillStyle(0);
    legend1_4->SetBorderSize(0);
    legend1_4->SetTextSize(0.05);
    legend1_4->AddEntry(g_an[1],"X-Ray I = 13 uA, V = 20 kV","");
    legend1_4->Draw();
    /*
    c0->cd(6);
    TH1D *h6 = new TH1D("h6","h6", 2000, -0.1, 1999.9);
    h6->SetTitle("Vin =  3750 V");
    h6->GetYaxis()->SetRangeUser(-0.2, 1.6);
    h6->GetXaxis()->SetTitle("Time [seconds]");
    h6->GetYaxis()->SetTitle("current [nA]");
    h6->Draw();
    fxray_an[6] = new TF1("fxray_an[6]", "pol0", 500, 800);
    fxray_cat[6] = new TF1("fxray_cat[6]", "pol0", 500, 800);
    fxray_an[6]->SetName("fxray_an[6]");
    fxray_cat[6]->SetName("fxray_cat[6]");
    g_an[6]->Fit("fxray_an[6]","R");
    g_cat[6]->Fit("fxray_cat[6]", "R");
    g_an[6]->Fit("ffe55_an6","R+");
    g_cat[6]->Fit("ffe55_cat6", "R+");
    g_an[6]->Draw("sameL");
    g_cat[6]->Draw("sameL");
    */
    /*
    ...
    h1->SetTitle("Vin =  3750 V, 06/04/2020");
    h1->GetYaxis()->SetRangeUser(-1.1, 2.20);
    h1->GetXaxis()->SetTitle("Time [seconds]");
    h1->GetYaxis()->SetTitle("current [nA]");
    h1->Draw();
    g1_an->Draw("sameL");
    ...
    h6->SetTitle("Vin =  3750 V, 06/03/2020");
    h6->GetYaxis()->SetRangeUser(-0.2, 0.6);
    h6->GetXaxis()->SetTitle("Time [seconds]");
    h6->GetYaxis()->SetTitle("current [nA]");
    h6->Draw();
    g6_an->Draw("sameL");
    ...
    */
    ...
    Float_t p0_xray_an[N],p0_xray_cat[N], p0_xray_sc[N], p0_fe55_an[N],p0_fe55_cat[N], p0_fe55_sc[N];
    Float_t IBF[N];
    Float_t xray_an_sc[N],xray_cat_sc[N], xray_screen_sc[N];

    for(int i = 0; i < 5; i++ ){
    p0_xray_an[i] = fxray_an[i+1]->GetParameter(0);
    p0_xray_cat[i] = fxray_cat[i+1]->GetParameter(0);
    p0_xray_sc[i] = fxray_sc[i+1]->GetParameter(0);
    p0_fe55_an[i] = ffe55_an[i+1]->GetParameter(0);
    p0_fe55_cat[i] = ffe55_cat[i+1]->GetParameter(0);
    p0_fe55_sc[i] = ffe55_cat[i+1]->GetParameter(0);

    IBF[i] = ((p0_xray_cat[i]) / (p0_xray_an[i]))*100.*(-1.0) ;

    cout << " x-ray anode :" << p0_xray_an[i]  << " x ray cathode " << p0_xray_cat[i] << " fe 55 an " << p0_fe55_an[i]  << " fe 55 cat " << p0_fe55_cat[i] << " xray screen :" << p0_xray_sc[i] << endl;
    cout << " IBF :" << IBF[i] << endl;
    xray_an_sc[i] = 1.0*p0_xray_an[i];
    xray_cat_sc[i] = 1.0*p0_xray_cat[i];
    xray_screen_sc[i] = 1.0*p0_xray_sc[i];
    }
    
    cout  << "voltages " << endl;
    const int n = 5;
    //New HVPS, drift = 1 kV/cm, transfer = induction = 2 kV/cm
    Float_t V[n] = {340, 345, 348, 352, 356}; // with screen and cathode+screen with HVPS fanout
    ...
    TGraph *xray_screen_cur = new TGraph(n , XrayI, xray_screen_sc);
    xray_screen_cur->SetName("xray_screen_cur");
    xray_screen_cur->SetMarkerStyle(20);
    xray_screen_cur->SetMarkerSize(2.0);
    xray_screen_cur->SetMarkerColor(6);
    xray_screen_cur->SetLineColor(2);
    ...
    legend0_1->AddEntry(xray_an_cur,"XRay operating at V = 20 kV for various I setting for effective Gain of 3500","");
    ...
    legend0_1->AddEntry(xray_screen_cur,"Measured current from Screen Cathode","PL");
    legend0_1->Draw();
    ...
    // ArCO2 (70:30)
    //Float_t sig[n] = {130, 165, 165, 204, 272, 360};
    Float_t sig[n] = {228, 360, 480, 720, 986}; // with screen and cathode+screen with HVPS fanout

    //ArCO2 : 80:20
    const int n3 =7 ;
    Float_t v2_quad[n3] = { 3300, 3350, 3375, 3400, 3425, 3450, 3500 };
    ...
    // eff_gain2[i] = (sig2[i]*slope_pA )/primary_arco2_80_20; //older Ar gas bottle
    }
    ...
    // hist0->GetYaxis()->SetTitleColor(2);
    hist0->Draw("");
    effgain->Draw("sameP");
    //c1->Update();
