...
//  ch.ls();
  ...
  TH1F *h1_qm = new TH1F("h1_qm","Max Amplitude Distribution", 4300, -1, 4299);
...
/*	 TTreeReader reader("fdctest", &f);
	 if (f->GetName().conatins("2057")) {
		TTreeReaderValue<float> dmcharge(reader, "qm_d");
        TTreeReaderValue<float> w2mcharge(reader, "qm_w2");
		
		bool firstEntry = true;
		while (reader.Next()) {
			if (firstEntry) {
         		// Check that branches exist and their types match our expectation.
         		if (!CheckValue(dmcharge)) return false;
         		if (!CheckValue(w2mcharge)) return false;
         		firstEntry = false;
      		}
			
			// Access the vector of values
      		if (!dmcharge.IsEmpty()&&!w2mcharge.IsEmpty()) {
         		// Access a float value - need to dereference as TTreeReaderValue
         		// behaves like an iterator
    			h1_qm->Fill(*dmcharge)
				h1_qm->Fill(*w2mcharge,"","same")
			}
