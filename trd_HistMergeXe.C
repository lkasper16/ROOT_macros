...
void trd_HistMergeXe(){
	
	TString rootFilesGEM[] = {"RootOutput/ps25/Run_006296_Output.root", "RootOutput/ps25/Run_006301_Output.root", "RootOutput/ps25/Run_006317_Output.root", "RootOutput/ps25/Run_006320_Output.root", "RootOutput/ps25/Run_006382_Output.root"};
  ...
  TString legendListGEM[] = {"5um Copper, Low Gain With Rad","5um Copper, High Gain With Rad", "5um Copper, Low Gain No Rad", "5um Copper, High Gain No Rad", "0.1um Aluminum, Low Gain With Rad","0.1um Aluminum, High Gain With Rad","","","",""};
  ...
  TString legendListURW[] = {"Low Gain With Rad","High Gain With Rad", "Low Gain No Rad", "High Gain No Rad", "High TF With Rad","","","","",""};
  
	TLegend *l1 = new TLegend(0.59, 0.65, 0.9, 0.9);
  TLegend *l2 = new TLegend(0.59, 0.65, 0.9, 0.9);
  ...
  //-- Triple GEM-TRD
	for (int i=0; i<sizeof(rootFilesGEM)/sizeof(rootFilesGEM[0]); i++) {
        ...
        l5->AddEntry(readObject, legendListURW[i], "lp");
      }
    }
  ...
  if (firstHist1) {
    ...
    firstHist1->SetTitle("GEMTRD ADC Distributions in Xe:CO2 90:10");
  }
  ...
  if (firstHist2) {
    ...
    firstHist2->SetTitle("MMG1-TRD ADC Distributions in Xe:CO2 90:10");
  }
 ...
 */
  ...
  TCanvas *c5 = new TCanvas("c5","uRWell-TRD ADC Distributions for XeCO2", 1600, 1000);
  c5->cd();
  gPad->SetLogy();
  gPad->SetGridx();
  gPad->SetGridy();
  ...
  if (firstHist5) {
    ...
    firstHist5->SetTitle("uRWell-TRD ADC Distributions in Xe:CO2 90:10");
  }
  histListURW->Draw("same");
  l5->Draw();
  c5->SaveAs("URWTRD_ADC_Xe_Comparison_v1.png");
